...
class Graph_Creator:
    ...
    #  Esize is the number of edges in the graph
    def CreateGraphWithRandomEdges(self, Esize):
        allTheEdges = self._FindAllTheEdges()
        edgeChoiceIndices = np.random.choice(
            len(allTheEdges), Esize, replace=False)
...
def main():
    graph = Graph_Creator()
    # Creates a random graph with 50 vertices and 200 edges
    graph.CreateGraphWithRandomEdges(200)
    # Reads the edges of the graph from a given CSV file
    graph.ReadGraphfromCSVfile("test_case")
